...
class SchulmanagerClient:
    ...
    def __init__(
        ...
        user_id: int | None = None,  # ← unterstützt Auswahl nach multipleAccounts
    ) -> None:
    ...
    async def _dump(self, name: str, data: Any) -> None:
        ...
        if not self.debug_dumps:
            return
        lname = name.lower()
        if "response" not in lname and "request" not in lname and "debug" not in lname:
            return
    ...
    # ---------- NEW: helper to inject institutionId ----------
    def _inject_institution(self, params: dict | None) -> dict:
        """Return a shallow-copied params dict with institutionId added.
        Also inject into nested 'student' dict if present.
        """
        p: dict[str, Any] = dict(params or {})
        iid = self._institution_id
        if iid is not None and "institutionId" not in p:
            p["institutionId"] = iid
        st = p.get("student")
        if isinstance(st, dict) and iid is not None and "institutionId" not in st:
            st2 = dict(st)
            st2["institutionId"] = iid
            p["student"] = st2
        return p
    # ---------------------------------------------------------

    async def _fetch_salt(self, email: str) -> str:
        """Fetch salt for password hashing (mirrors browser behavior)."""
        ...
        payload = {
            ...
            "userId": self._user_id,   # nach Schulauswahl sendet Browser userId
            "institutionId": None,
        }

        await self._dump(
            "get_salt_request.json",
            {"url": GET_SALT_URL, "headers": sanitize_for_log(headers), "payload": payload},
        )
        ...
        async with sess.post(GET_SALT_URL, json=payload, headers=headers) as resp:
            ...
            try:
                ...
                await self._dump("get_salt_response.json", {"status": resp.status, "raw": text[:500]})
                raise RuntimeError(f"get_salt_failed:{resp.status}")

        await self._dump("get_salt_response.json", {"status": 200, "salt_len": len(salt)})
        return salt
    ...
    async def async_login(self) -> None:
        ...
        await self._dump(
            "login_request.json",
            {"url": LOGIN_URL, "headers": sanitize_for_log(headers), "payload": sanitize_for_log(body)},
        )
        ...
        async with sess.post(LOGIN_URL, json=body, headers=headers) as resp:
            ...
            try:
                ...
                await self._dump("login_response.json", {"status": resp.status, "raw": text[:500]})
                raise RuntimeError(f"login_failed:{resp.status}")

            await self._dump("login_response.json", {"status": resp.status, "data": sanitize_for_log(data)})
            ...
            if "multipleAccounts" in data:
                ...
                _LOGGER.debug("Multi-school account detected with %d schools", len(self._multiple_accounts))
                return
            ...
            for p in parents:
                ...
                if not sid:
                    continue
                name = f"{st.get('firstname', '')} {st.get('lastname', '')}".strip() or "Schüler"
                self._students.append({"id": str(sid), "classId": st.get("classId"), "name": name})

            if not self._students:
                _LOGGER.debug("No students found in login response; trying fallback via /api/calls")
                try:
                    await self._fallback_discover_students()
                except Exception as e:
                    _LOGGER.debug("Fallback student discovery failed: %s", e)
    ...
    async def _discover_bundle_version(self) -> str | None:
        ...
        async with sess.get(INDEX_URL, headers=headers_html) as resp:
            ...
            await self._dump("index_meta.json", {"status": resp.status, "length": len(html)})
        ...
        near_pat = re.compile(r'bundleVersion[\s\S]{0,120}?["\']([a-f0-9]{10})["\']', re.IGNORECASE)
        ...
        for url in scripts:
            try:
                ...
                if m_lit:
                    ...
                    await self._dump("script_probe_hit.json", {"url": url, "mode": "literal", "value": val})
                    return val

                m_ident = ident_pat.search(js)
                if m_ident:
                    ident = m_ident.group(1)
                    def_pat = re.compile(rf'\b(?:const|let|var)\s+{re.escape(ident)}\s*=\s*["\']([a-f0-9]{{10}})["\']')
                    m_def = def_pat.search(js)
                    if m_def:
                        val = m_def.group(1)
                        self._bundle_version = val
                        await self._dump("script_probe_hit.json", {"url": url, "mode": f"ident:{ident}", "value": val})
                        return val
                ...
                if m_near:
                    val = m_near.group(1)
                    self._bundle_version = val
                    await self._dump("script_probe_hit.json", {"url": url, "mode": "near", "value": val})
                    return val
    ...
    async def _api_call(self, module: str, endpoint: str, parameters: dict, tag: str) -> Any:
        ...
        # inject institutionId (and into nested student)
        parameters = self._inject_institution(parameters)
        ...
        await self._dump(f"calls_request_{tag}.json", {"headers": sanitize_for_log(headers), "payload": payload})
        ...
        async with sess.post(CALLS_URL, json=payload, headers=headers) as resp:
            ...
            try:
                ...
                data = {"raw_text": text[:500], "content_type": resp.headers.get("Content-Type")}

            await self._dump(f"calls_response_{tag}.json", {"status": status, "headers": dict(resp.headers), "body": data})

            if status != 200:
                _LOGGER.warning("API call failed: %s/%s -> %s, Response: %s", module, endpoint, status, data)
                raise RuntimeError(f"/api/calls {module}/{endpoint} -> {status}")

            results = data.get("results") or []
            if not results:
                _LOGGER.warning("No results in API response for %s/%s", module, endpoint)
                return []
            ...
            if isinstance(result, dict) and "status" in result and result["status"] != 200:
                _LOGGER.warning(
                    ...
                    module, endpoint, result.get("status"), result.get("id"),
                )
                return []

            return (result.get("data", []) if isinstance(result, dict) and "data" in result else data)

    async def _fallback_discover_students(self) -> None:
        """Fallback: Versuche Schüler via /api/calls zu ermitteln, wenn der Login-Response keine liefert."""
        candidates = [
            ("family",   "get-children",          {}),
            ("parents",  "get-children",          {}),
            ("students", "get-linked-students",   {}),
            ("users",    "get-linked-students",   {}),
            ("students", "get-students-for-user", {"userId": None}),
            ("profiles", "get-children",          {}),
        ]

        attempts: list[dict[str, Any]] = []

        def _extract_students(obj: Any) -> list[dict[str, Any]]:
            results: list[dict[str, Any]] = []

            def _try_add(st: dict[str, Any]) -> None:
                sid = st.get("id") or (st.get("student") or {}).get("id")
                if not sid:
                    return
                if "firstname" in st or "lastname" in st:
                    first = st.get("firstname", "")
                    last = st.get("lastname", "")
                else:
                    s2 = st.get("student") or {}
                    first = s2.get("firstname", "")
                    last = s2.get("lastname", "")
                name = f"{first} {last}".strip() or "Schüler"
                class_id = st.get("classId") or (st.get("student") or {}).get("classId")
                results.append({"id": str(int(sid)), "classId": class_id, "name": name})
            ...
            if isinstance(obj, dict):
                for key in ("children", "students", "data", "items", "list"):
        ...
        for module, endpoint, params in candidates:
            try:
                data = await self._api_call(module, endpoint, params, f"fallback_{module}_{endpoint}")
                attempts.append({"module": module, "endpoint": endpoint, "ok": True, "count_hint": (len(data) if isinstance(data, list) else None)})
                extracted = _extract_students(data)
                if extracted:
                    existing_ids = {s["id"] for s in self._students}
                    for st in extracted:
                        if st["id"] not in existing_ids:
                            self._students.append(st)
                    break
            except Exception as e:
                attempts.append({"module": module, "endpoint": endpoint, "ok": False, "error": str(e)})
                continue

        await self._dump("students_fallback_attempts.json", attempts)
        await self._dump("students_extracted.json", self._students)

    async def fetch_homework(self, student_id: str) -> list[dict]:
        """Fetch homework for a student."""
        params = {"student": {"id": int(student_id)}}
        data = await self._api_call("classbook", "get-homework", params, f"homework_{student_id}_classbook_get-homework")
        await self._dump(f"hausaufgaben_{student_id}.json", data)
        return data if isinstance(data, list) else []

    async def fetch_exams(self, student_id: str, class_id: int | None = None, date_range_config: dict[str, int] | None = None) -> list[dict]:
        """Fetch exams for a student using the proper exams API with user-configured date range."""
        student_info = next((s for s in self._students if s["id"] == student_id), None)
        if not student_info:
        ...
        # inject institutionId into parameters and nested student
        parameters = self._inject_institution({
            "student": student_obj,
        ...
        async with sess.post(CALLS_URL, json=batch_payload, headers=headers) as resp:
            ...
            await self._dump(f"exams_response_{student_id}.json", {"status": resp.status, "data": response_data})
    ...
    async def fetch_schedule_today_tomorrow(self, student_id: str, class_id: int | None = None, weeks: int = 2) -> SchedulePayload:
    ...
    async def _fetch_schedule_optimized(self, student_id: str, weeks: int = 2) -> SchedulePayload:
        ...
        if not student_info:
            _LOGGER.warning("Student info not found for ID %s", student_id)
            return {"today": [], "tomorrow": [], "week": {}, "changes": {"today": [], "tomorrow": [], "summary": "Keine Stundenplanänderungen für heute und morgen"}}
        ...
        async with sess.post(CALLS_URL, json=batch_payload, headers=headers) as resp:
            ...
            response_data = await resp.json()
            await self._dump(f"schedule_batch_response_{sid}.json", {"status": resp.status, "data": response_data})

            return self._parse_batch_schedule_response(response_data, today)

    def _parse_batch_schedule_response(self, response_data: dict[str, Any], reference_date: date) -> SchedulePayload:
    ...
    def _parse_schedule_data(self, data: Any, reference_date: date) -> SchedulePayload:
        ...
        if isinstance(data, list):
            ...
            items = (data.get("lessons") or data.get("data") or data.get("schedule") or data.get("timetable") or [])
        else:
            return {"today": [], "tomorrow": [], "week": {}, "changes": {"today": [], "tomorrow": [], "summary": "Keine Stundenplanänderungen für heute und morgen"}}
        ...
        return {"today": tlist, "tomorrow": nlist, "week": week_map, "changes": {"today": [], "tomorrow": [], "summary": "Keine Stundenplanänderungen für heute und morgen"}}
    ...
    async def _fetch_subjects(self) -> dict[int, dict[str, Any]]:
        ...
        # add institutionId also for this POQA call
        parameters = self._inject_institution({
            "action": {
                "model": "main/subject",
                "action": "findAll",
        ...
        try:
            async with session.post(CALLS_URL, json=request_data, headers=headers) as response:
                if response.status != 200:
                    _LOGGER.error("Failed to fetch subjects: HTTP %d", response.status)
                    return {}

                data = await response.json()
                await self._dump("subjects_response.json", data)
                ...
                subjects_cache: dict[int, dict[str, Any]] = {}
    ...
    async def fetch_grades(self, student_id: str, class_id: int | None = None) -> GradesPayload:
        ...
        term_id = 28592  # TODO: dynamisch ermitteln
        ...
        async with sess.post(CALLS_URL, json=grades_payload, headers=headers) as response:
            if response.status != 200:
                ...
                return {"subjects": {}, "overall_average": None, "total_subjects": 0, "subjects_with_grades": 0}
    ...
    async def _process_grades_data(self, grades_data: dict) -> GradesPayload:
        ...
        course_map: dict[int, dict[str, Any]] = {int(course["id"]): course for course in courses if "id" in course}
        ...
        for event in grading_events:
            ...
            if subject_id not in subjects:
                ...
                subjects[subject_id] = {"name": subject_name, "abbreviation": subject_abbrev, "average": None, "grades": {}}
        ...
        return cast(GradesPayload, {"subjects": subjects, "overall_average": overall_average, "total_subjects": len(subjects), "subjects_with_grades": len(all_subject_averages)})
    ...
    async def async_update(self, enabled_features: dict[str, bool] | None = None, date_range_config: dict[str, int] | None = None) -> dict[str, Any]:
        ...
        if enabled_features is None:
            enabled_features = {"homework": True, "schedule": True, "exams": True, "grades": True}

        result: dict[str, Any] = {"students": list(self._students), "homework": {}, "schedule": {}, "exams": {}, "grades": {}}
        ...
        for st in self._students:
            ...
            if enabled_features.get("homework", True):
                try:
                    ...
                    _LOGGER.warning("Schulmanager: homework fetch failed for %s: %s", sid, err)
                    result["homework"][sid] = []
            else:
                result["homework"][sid] = []
            ...
            if enabled_features.get("schedule", True):
                try:
                    ...
                    _LOGGER.warning("Schulmanager: schedule fetch failed for %s: %s", sid, err)
                    result["schedule"][sid] = {"today": [], "tomorrow": [], "week": {}, "changes": {"today": [], "tomorrow": [], "summary": "Keine Stundenplanänderungen für heute und morgen"}}
            else:
                result["schedule"][sid] = {"today": [], "tomorrow": [], "week": {}, "changes": {"today": [], "tomorrow": [], "summary": "Keine Stundenplanänderungen für heute und morgen"}}
...
class MultiSchoolClient:
    ...
    def __init__(self, hass: HomeAssistant, username: str, password: str, debug_dumps: bool = False) -> None:
        self.hass = hass
        self.username = username
        self.password = password
        self.debug_dumps = debug_dumps
        self.clients: dict[int, SchulmanagerClient] = {}  # school userId -> client
        self.school_names: dict[int, str] = {}            # school userId -> school_name
        self._all_students: list[dict[str, Any]] = []

    async def async_login_all_schools(self, schools: list[dict[str, Any]]) -> None:
        ...
        async def login_to_school(school: dict[str, Any]) -> tuple[int, str, SchulmanagerClient]:
            ...
            _LOGGER.debug("Logging into school: %s (userId: %d)", school_name, school_user_id)
            ...
            return school_user_id, school_name, client

        login_tasks = [login_to_school(s) for s in schools]
        ...
        for result in results:
            ...
            school_user_id, school_name, client = result
            self.clients[school_user_id] = client
            self.school_names[school_user_id] = school_name

            students = client.get_students()
            for student in students:
                student_with_school = student.copy()
                student_with_school["school_id"] = school_user_id
                student_with_school["school_name"] = school_name
                self._all_students.append(student_with_school)

            _LOGGER.info("Logged into school '%s': %d students found", school_name, len(students))

        _LOGGER.info("Multi-school login complete: %d schools, %d total students", len(self.clients), len(self._all_students))
    ...
    async def async_update(self, enabled_features: dict[str, bool] | None = None, date_range_config: dict[str, int] | None = None) -> dict[str, Any]:
        result: dict[str, Any] = {"students": list(self._all_students), "homework": {}, "schedule": {}, "exams": {}, "grades": {}}
